...
{
//	//	按键使用浮空输入模式
//	SET_BIT( RCC->APB2ENR, RCC_APB2ENR_IOPEEN ); 						// 打开E端口的时钟
//	GPIOA->BSRR = 0x007Eu; 																	// E端口的1--6输出为1；
//	MODIFY_REG( GPIOA->CRL, 0x0FFFFFF0u, 0x04444440u );			//	配置E端口的1--6为浮空输入
...
//	CLEAR_BIT( EXTI->EMR,  0x2F7Eu );	// no event 无中断事件
////	SET_BIT  ( EXTI->RTSR, 0x2F7Eu );	// rising edge trigger  上升沿触发
//	SET_BIT  ( EXTI->FTSR, 0x2F7Eu );	// falling edge trigger 下降沿触发
//	WRITE_REG( EXTI->PR,   0xFFFFu );	// 通过写1能复位触发的中断标志

//	SET_BIT  ( EXTI->IMR,  0x007Eu );	// 允许中断 #1~#6 #8~#13。
//	// IO中断	
...
*******************************************************************************/
	...
	*	吹气电机端口设置 TIM3->CH3
	*******************************************************************************/
		void	BIOS_Air_Charge_Init( void )
		{
			...
			TIM3->PSC		= SystemCoreClock / 100000u - 1u;										//	设置预分频值
...
//			SET_BIT( TIM3->CR1, TIM_CR1_CEN );																		//	使能计数器
	...
	*	吹气电机PWM控制
	*******************************************************************************/
		void	Air_Charge_CTRL( uint16_t ARRValue, uint16_t CCR3Value )
		{
...
//		}
	...
	*	电机方向控制
	*******************************************************************************/
		void	Air_Charge_DIR( _Bool	State )
		{
			if( State )
				SET_BIT( GPIOB->BSRR, GPIO_BSRR_BS10 );
			else
	...
	*	电机驱动器使能 
	*******************************************************************************/
		void	Air_Charge_EN( _Bool State )
		{
			if( State )
	...
	*******************************************************************************/
		void	BIOS_Air_Bleed_Init( void )
		{
			...
			TIM4->PSC		= SystemCoreClock / 100000u - 1u;											//	设置预分频值
...
//		}
	...
	*	旋转舵机端口设置 TIM5->CH1
	*******************************************************************************/
		void	BIOS_Rotate_Servo_Init( void )
		{
...
//			SET_BIT( GPIOB->BSRR, GPIO_BSRR_BR15);
			...
			TIM5->CR1   = TIM_CR1_ARPE;																						//	自动装载
			TIM5->PSC		= SystemCoreClock / 1000000u - 1u;												//	设置预分频值
			TIM5->ARR   = 20000u;																									//	设置预装载值
			TIM5->CCMR1 = TIM_CCMR1_OC1M_1 | TIM_CCMR1_OC1M_2 | TIM_CCMR1_OC1PE;	//	设置计数极性和工作模式
			TIM5->CCER  = TIM_CCER_CC1E | TIM_CCER_CC1P;;													//	使能通道1
			TIM5->CCR1  = 0u;																											//	设置翻转值
//			TIM5->DIER  = TIM_DIER_UIE;																					//	使能中断;																					//	使能1号中断
//			CLEAR_BIT( TIM5->SR, TIM_SR_UIF );																	//	清除中断标记
...
//			SET_BIT( TIM5->CR1, TIM_CR1_CEN );																		//	使能计数器
	...
	*	旋转舵机PWM控制
	*******************************************************************************/
		void	Rotate_Servo_CTRL( uint16_t ARRValue, uint16_t CCR1Value )
		{
	...
	*	旋转舵机PWM开关
	*******************************************************************************/
		void	Rotate_Servo_CMD( _Bool	CMD )
		{
			if( CMD )
			{
	...
	*	电机方向控制
	*******************************************************************************/
//		void	Rotate_Servo_DIR( _Bool	State )
//		{
//			if( State )
//				SET_BIT( GPIOB->BSRR, GPIO_BSRR_BS12 );
//			else
...
//		}
	...
	*	电机驱动器使能 
	*******************************************************************************/
//		void	Rotate_Servo_EN( _Bool State )
//		{
//			if( State )
  	...
  	void	BIOS_Fingers_Init( void )
		{
			...
			MODIFY_REG( GPIOC->CRH, 0x000000F0u, 0x000000B0u );										//	IO口配置		PWM_OUT
//			SET_BIT( GPIOC->BSRR, GPIO_BSRR_BS9 );
			SET_BIT( RCC->APB2ENR, RCC_APB2ENR_IOPAEN );
			MODIFY_REG( GPIOA->CRH, 0x0000000Fu, 0x00000003u );										//	IO口配置		Fingers_DIR(Cmd)
			...
			TIM8->CCR1  = 0u;																											//	设置翻转值
			TIM8->DIER  = TIM_DIER_UIE;																						//	使能中断
			NVIC_EnableIRQ( TIM8_UP_IRQn );
			CLEAR_BIT( TIM8->SR, TIM_SR_UIF);											//	清除中断标记
//			SET_BIT( TIM8->CR1, TIM_CR1_CEN );																		//	使能计数器
  ...
  *******************************************************************************/
  	void	BIOS_Fingers_PWMIn_Init( void )
		...
		#define	PWMInCountErr 5u
		uint32_t	PWMInCount( uint32_t	CountTarget )
		{
			if( CountTarget )
			{
				if( TIM1->CNT - CountTarget < PWMInCountErr )
					return true;
...
*******************************************************************************/
    ...
    *******************************************************************************/
    _Bool	LevelSwitch1Read( void )
		{		
			return	READ_BIT( GPIOA->IDR, GPIO_IDR_IDR5 );
		}
   /******************************** 功能说明 *************************************
    *	液位检测2
    *******************************************************************************/
...
int	BIOS_Init( void )
{
	...
	BIOS_Air_Charge_Init();
	BIOS_Air_Bleed_Init();

	BIOS_Rotate_Servo_Init();

	BIOS_Fingers_Init();
	BIOS_Fingers_PWMIn_Init();
