...
*	ADC转换
*******************************************************************************/
volatile	uint16_t 	ADC_Databuf[12u];	//	ADC-DMA转换数据缓冲区
void	BIOS_ADC_Init( void )
{
	ADC_DeInit( ADC1 );
	SET_BIT(RCC->APB2ENR,RCC_APB2ENR_IOPAEN);
	MODIFY_REG( GPIOA->CRL, 0xFFF00000u, 0x00000000u );			//	配置端口为浮空输入
...
//						 | ( (8u)<< 15u )
						 ; //	ADC转换通道选择
	ADC1->SMPR2 = ADC_SMPR2_SMP5 | ADC_SMPR2_SMP6 | ADC_SMPR2_SMP7;	//	
	ADC1->SQR1 |=( (3u)/*括号内是通道数量*/- 1u ) << 20u;			//	ADC转换序列总长度
	...
	*/
	...
	delay_us(10);
	SET_BIT( ADC1->CR2, ADC_CR2_ADON ); 	//	ADC开关控制
	SET_BIT( ADC1->CR2, ADC_CR2_SWSTART ); 	//	ADC开关控制
...
//	//	按键使用浮空输入模式
	...
	GPIOB->BSRR |= 0x0018u; 																// 端口输出为1;
	MODIFY_REG( GPIOB->CRL, 0x000FF000u, 0x00044000u );			//	配置端口为浮空输入
...
//	SET_BIT  ( EXTI->RTSR, 0x0018u );	// rising edge trigger  上升沿触发
	SET_BIT  ( EXTI->FTSR, 0x0018u );	// falling edge trigger 下降沿触发
	WRITE_REG( EXTI->PR,   0xFFFFu );	// 通过写1能复位触发的中断标志

	SET_BIT  ( EXTI->IMR,  0x0018u );	// 允许中断
//	// IO中断	
...
void	EXTIx_IRQ_Disable( uint8_t EXTIx )
{
	...
	WRITE_REG( EXTI->PR,   0x01u << EXTIx );	// 写1复位中断标志位
}
...
//			SET_BIT( TIM4->CR1, TIM_CR1_CEN );																		//	使能计数器
	...
	*********************************************************************************/
		...
		*	水冷泵端口设置 TIM5->CH1
		*******************************************************************************/
			void	BIOS_CoolerPump( void )
			{
				SET_BIT( RCC->APB2ENR, RCC_APB2ENR_IOPAEN );
				MODIFY_REG( GPIOA->CRL, 0x0000000Fu, 0x0000000Bu );										//	IO口配置		PWM_OUT
				SET_BIT( RCC->APB2ENR, RCC_APB2ENR_IOPCEN );                          
				MODIFY_REG( GPIOC->CRL, 0x0000000Fu, 0x00000003u );										//	IO口配置		EN
...
//				SET_BIT( TIM5->CR1, TIM_CR1_CEN );																		//	使能计数器
		...
		*	水冷泵脉冲计数
		*******************************************************************************/
			void	BIOS_Cooler_PWMIn_Init( void )
			{
				SET_BIT( RCC->APB2ENR, RCC_APB2ENR_IOPAEN );
		...
		*		水冷泵PWM控制
		*******************************************************************************/
			void	PWM_CTRL_CoolerPump( uint16_t ARRValue, uint16_t CCR1Value )
			{
				TIM5->ARR   =  ARRValue - 1;					//	设置预装载值
				TIM5->CCR1  =  CCR1Value;					//	设置翻转值
			}
		/******************************** 功能说明 *************************************
		*		水冷泵PWM开关
		*******************************************************************************/
			void	PWM_CMD_CoolerPump( _Bool	CMD )
			{
				if( CMD )
				{
...
//			}
		...
		*	电机驱动器使能 
		*******************************************************************************/
			void	CoolerPump_EN( _Bool State )
			{
				if( State )
					SET_BIT( GPIOC->BSRR, GPIO_BSRR_BS0 );
				else
...
//			SET_BIT( GPIOC->BSRR, GPIO_BSRR_BS9 );
			...
			TIM8->CR1   = TIM_CR1_ARPE ;																					//	自动装载 仅溢出中断
			TIM8->PSC		= SystemCoreClock / 10000u - 1u;														//	设置预分频值
			TIM8->ARR   = 10000u;																									//	设置预装载值
			TIM8->CCMR2 = TIM_CCMR2_OC4M_1 | TIM_CCMR2_OC4M_2 | TIM_CCMR2_OC4PE;	//	设置计数极性和工作模式
			TIM8->CCER  = TIM_CCER_CC4E ;														//	使能通道4
			TIM8->CCR1  = 0u;																											//	设置翻转值
//			TIM8->DIER  = TIM_DIER_CC4IE;																					//	使能1号中断
...
//			SET_BIT( TIM8->CR1, TIM_CR1_CEN );																		//	使能计数器
  ...
  *	加热消解PWM控制
  *******************************************************************************/
		void	Heat_CTRL( uint16_t ARRValue, uint16_t CCR1Value )
		{
